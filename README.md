...
Сhannels (Внешние точки входа) - Мессенджеры, почта, календарь, веб-интерфейс
Gateway (Приём и нормализация) - Webhook парсер, аутентификация, rate-limiting, очередь событий
Orchestrator Мозг системы - Event loop, AI-роутер, сбор контекста, управление сессией
LLM Генерация решений - Подключение к моделям, шаблонизация промптов, трекинг токенов
Skill Engine - Исполнение действий - Реестр навыков, синхронизация с ClawHub, песочница, гейт подтверждений
Memory Хранение состояния - Векторная БД (RAG/контекст), KV (сессии), реляционная БД (логи/задачи), кэш
Security - Защита и наблюдаемость - RBAC/scopes, аудит, метрики, обработка ошибок и fallback-механизмы
Actions Внешние интеграции - MCP-мост, CRM, облака, сторонние API
...
При первом запуске агента (BOOTSTRAP) происходит следующий процесс :

Инициализация рабочей области — создаются все необходимые файлы (AGENTS.md, SOUL.md, IDENTITY.md, USER.md и др.)
Короткий Q&A ритуал — агент задает несколько вопросов пользователю (имя, предпочтения, таймзона)
Заполнение профилей — ответы записываются в IDENTITY.md, USER.md, SOUL.md
...
Проверить валидность конфига (если есть такая команда)
...
Dashboards: latency, token cost, tool success rate, error rate, queue depth
...
# Выводы
- Пожиратель токенов номер 1 в мире! https://openrouter.ai/rankings 
