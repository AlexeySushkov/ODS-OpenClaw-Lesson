# OpenClaw: личный ИИ-агент в рабочей среде (14.04.2026)
...
Алексей Сушков также участвует в развитии сообщества ИИ, реализует собственные pet-проекты, делится результатами исследований в [**статьях на Habr**](https://habr.com/ru/users/AlexeySushkov/)

## План семинара
- Кейсы OpenClaw
- Кейс для семинара: агент, который разбирает дела и осуществляет поиск информации (morning briefing)
...
- При первом запуске агента (BOOTSTRAP) заполняются файлы:
  - BOOTSTRAP.md - запускает процесс, потом удаляется
  - IDENTITY.md - имя агента, стиль, emoji, аватар
  - USER.md - про меня: как обращаться, таймзона
  - SOUL.md - про агнта: границы, тон общения
...
```
- Отключить постоянно: cконфигурировать Heartbeat (~/.openclaw/openclaw.json)
```json
      "heartbeat": {
        "every": "0m",
        "target": "none"
...
```
Посмотрите логи — должна быть запись с новой моделью:
```bash
